...
// Switching FROM clears stale clause projections: SELECT / WHERE / GROUP BY
// reference tags of the previous resource, so a lighter patch (resource only)
// would leave tags that do not exist on the new schema in the generated BydbQL
// (invalid BydbQL or a confusing UI state). Mirrors the query-console
// onPickResource reset.
describe('QueryBuilder FROM — clause reset on resource/group switch', () => {
  const baseProps = (
    state: QBBuilderState,
    resourceNames: string[],
    groupNames = ['g1'],
  ): QueryBuilderProps & { onChange: ReturnType<typeof vi.fn> } => ({
    state,
    onChange: vi.fn(),
    tags: ['name', 'message'],
    fields: [],
  ...
  function staleState(): QBBuilderState {
    return {
      ...stateWithLeaf({ tag: 'name', op: 'BINARY_OP_EQ', value: 'nodea' }),
      projection: ['entity_id'],
      groupBy: ['entity_id'],
      select: [{ field: 'count', fn: 'SUM' }],
    };
  }

  it('resets SELECT projection / WHERE / GROUP BY when the resource changes', () => {
    const { onChange } = baseProps(staleState(), ['logs', 'events']);
    render(<MemoryRouter><QueryBuilder
      state={staleState()}
      onChange={onChange}
      tags={['name', 'message']}
      fields={[]}
      groupNames={['g1']}
      resourceNames={['logs', 'events']}
      topnAggNames={[]}
      groups={[]}
      groupResources={new Map()}
      groupTopnAggs={new Map()}
      ...
      setOpenSection={vi.fn()}
    /></MemoryRouter>);
    fireEvent.change(screen.getByLabelText('Resource'), { target: { value: 'events' } });
    const call = onChange.mock.calls.at(-1)?.[0];
    expect(call).toMatchObject({ resource: 'events', projection: [], groupBy: [], select: [] });
    expect(call.where.children).toEqual([]);
  });

  it('resets clause projections and clears the resource when the group changes', () => {
    const { onChange } = baseProps(staleState(), ['logs'], ['g1', 'g2']);
    render(<MemoryRouter><QueryBuilder
      state={staleState()}
      onChange={onChange}
      tags={['name', 'message']}
    ...
    const call = onChange.mock.calls.at(-1)?.[0];
    expect(call).toMatchObject({ group: 'g2', resource: '', projection: [], groupBy: [], select: [] });
    expect(call.where.children).toEqual([]);
  });
});
