...
func writeWorkflow(prompt *bytes.Buffer) {
	prompt.WriteString("First decide what the user actually asked for:\n")
	prompt.WriteString("- A schema, capability, or usage question is answered in words. ")
	prompt.WriteString("Use list_groups_schemas and describe_schema to look it up, then answer. ")
	prompt.WriteString("Do not call propose_query_plan or execute_bydbql, because reading stored rows is not what was asked.\n")
	prompt.WriteString("- A request for stored data follows the controlled query workflow below.\n")
	prompt.WriteString("- When a turn is ambiguous, prefer answering and ask one clarification instead of reading data.\n\n")
	...
	prompt.WriteString("5. For a data request, submit through propose_query_plan and do not finish that turn until it returns valid=true.\n")
	...
	prompt.WriteString("8. execute_bydbql accepts only the exact latest successfully compiled statement, and runs it immediately.\n")
	...
	prompt.WriteString("11. For a repair, preserve valid plan parts while fixing the structured validation or execution diagnostic.\n\n")
}
...
func writeHardRules(prompt *bytes.Buffer) {
	...
	prompt.WriteString("- Compiling a plan never executes it. Only execute_bydbql reads data.\n")
	...
	prompt.WriteString("- TRACE plans need a scan entry point: order_by.index_rule from sortable_indexes, ")
	prompt.WriteString("or an equality or IN filter on the trace ID tag named by plan_constraints.trace_scan_requirement. ")
	prompt.WriteString("A TRACE plan with neither is rejected, because BanyanDB cannot plan the scan.\n")
	...
	prompt.WriteString("- Repair validation_error or execution_summary.error when present.\n")
	prompt.WriteString("- Execution previews contain at most 50 rows and can inform only a new independently validated plan.\n\n")
}
