...
func TestProxyClient_Connect_Success(t *testing.T) {
	testLogger := initTestLogger(t)
	fr := flightrecorder.NewFlightRecorder(1000000)
	pc := NewProxyClient("localhost:8080", "192.168.1.1", 9090, "worker", "test", "worker", nil, 5*time.Second, 10*time.Second, fr, testLogger)
...
func TestProxyClient_StartRegistrationStream_Success(t *testing.T) {
	testLogger := initTestLogger(t)
	fr := flightrecorder.NewFlightRecorder(1000000)
	pc := NewProxyClient("localhost:8080", "192.168.1.1", 9090, "worker", "test", "worker", map[string]string{"env": "test"}, 5*time.Second, 10*time.Second, fr, testLogger)
...
func TestProxyClient_Start_ContextCancellation(t *testing.T) {
	testLogger := initTestLogger(t)
	fr := flightrecorder.NewFlightRecorder(1000000)
	pc := NewProxyClient("invalid-address:99999", "192.168.1.1", 9090, "worker", "test", "worker", nil, 5*time.Second, 100*time.Millisecond, fr, testLogger)
