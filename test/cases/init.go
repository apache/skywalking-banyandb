...
func Initialize(addr string, now time.Time) {
	...
	casesmeasuredata.Write(conn, "service_instance_endpoint_cpm_minute", "sw_metric", "endpoint_resp_time_minute_data.json", now.Add(20*time.Second), interval)
	casesmeasuredata.Write(conn, "service_instance_endpoint_cpm_minute", "sw_metric", "endpoint_resp_time_minute_data2.json", now.Add(20*time.Minute), interval)
