...
func TestPropertyRepairHalfData(t *testing.T) {
	...
	ginkgo.RunSpecs(t, "Property Repair Half Data Test Suite", ginkgo.Label("integration", "property_repair", "half_data"))
}
...
var _ = ginkgo.Describe("Property Repair Half Data Test", ginkgo.Ordered, func() {
	ginkgo.Describe("Step 1: Initial Setup and Data Load", func() {
		...
		ginkgo.It("Should create group with 2 replicas and write 50k properties", func() {
			ctx := context.Background()

			fmt.Println("=== Step 1: Creating group with 2 replicas and loading initial data ===")
			...
			fmt.Printf("=== Step 1 completed: wrote 50,000 properties in %v ===\n", duration)
		})
	})
	...
	ginkgo.Describe("Step 2: Reduce Replicas and Add More Data", func() {
		ginkgo.It("Should reduce group replicas to 1", func() {
			...
			duration := time.Since(startTime)
			fmt.Printf("=== Step 2 completed: updated replicas to 1 in %v ===\n", duration)
		})
		...
		ginkgo.It("Should write additional 5k properties", func() {
			...
			duration := time.Since(startTime)
			fmt.Printf("=== Step 3 completed: wrote additional 50,000 properties in %v ===\n", duration)
		})
		...
		ginkgo.It("Should increase group replicas back to 2", func() {
			...
			duration := time.Since(startTime)
			fmt.Printf("=== Step 4 completed: updated replicas to 2 in %v ===\n", duration)
		})
	})
