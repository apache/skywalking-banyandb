...
var _ = g.Describe("Trace Normal Mode Replication", func() {
	...
	g.It("should return consistent results from replicas", func() {
		...
		resp, err := schemaClient.Get(ctx, &databasev1.TraceRegistryServiceGetRequest{Metadata: traceMetadata})
		gm.Expect(err).NotTo(gm.HaveOccurred())
		gm.Expect(resp.GetTrace()).NotTo(gm.BeNil())
		gm.Expect(resp.GetTrace().GetMetadata().GetGroup()).To(gm.Equal("test-trace-group"))

		g.By("Verifying replication factor for test-trace-group")
		...
		verifyTraceDataContentWithArgs(conn, now, helpers.Args{
			Input:    "all",
...
func verifyTraceDataContentWithArgs(conn *grpc.ClientConn, baseTime time.Time, args helpers.Args) {
