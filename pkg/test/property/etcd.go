...
const (
	propertyDir = "testdata/property"
)

var (
	//go:embed testdata/property/*.json
...
func loadSchemas(ctx context.Context, e schema.Registry) error {
	streams, err := propertyStore.ReadDir(propertyDir)
	if err != nil {
		return err
	}
	var data []byte
	for _, entry := range streams {
		data, err = propertyStore.ReadFile(path.Join(propertyDir, entry.Name()))
		if err != nil {
			return err
		}
