...
var _ = Describe("Metadata", func() {
	...
	Context("Manage group", func() {
		...
		It("should close the group", func() {
			svcs.repo.EXPECT().Publish(event.StreamTopicShardEvent, &shardEventMatcher{action: databasev1.Action_ACTION_DELETE}).Times(2)
		...
		It("should add shards", func() {
			svcs.repo.EXPECT().Publish(event.StreamTopicShardEvent, &shardEventMatcher{action: databasev1.Action_ACTION_DELETE}).Times(2)
			svcs.repo.EXPECT().Publish(event.StreamTopicShardEvent, &shardEventMatcher{action: databasev1.Action_ACTION_PUT}).Times(4)
	...
	Context("Manage stream", func() {
		...
		It("should close the stream", func() {
			svcs.repo.EXPECT().Publish(event.StreamTopicEntityEvent, &entityEventMatcher{action: databasev1.Action_ACTION_DELETE}).Times(1)
		...
		Context("Update a stream", func() {
			...
			It("should update a new stream", func() {
				svcs.repo.EXPECT().Publish(event.StreamTopicEntityEvent, &entityEventMatcher{action: databasev1.Action_ACTION_PUT}).Times(1)
