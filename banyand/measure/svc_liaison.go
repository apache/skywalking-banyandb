...
func (s *liaison) PreRun(ctx context.Context) error {
	...
	// (G9f.5.c's distributed plan recognizes the []byte shape and runs
	// ReduceFramesToInternalDataPoints / DecodeFramesToInternalDataPoints).
	...
	if subscribeErr := s.pipeline.Subscribe(data.TopicMeasureDropGroup, &dropGroupLiaisonListener{s: s}); subscribeErr != nil {
