...
func TestCollectAllSrcGroupRoots_LiveDedup(t *testing.T) {
	...
	cfg := DirectCopyConfig{
		Groups: []string{group},
		Entries: []DirectCopyEntry{
			// Two entries both pointing at hot-0 — must dedup.
			{Stage: "hot", Target: filepath.Join(tmp, "out-a"), Source: []string{hot0}},
			{Stage: "hot", Target: filepath.Join(tmp, "out-b"), Source: []string{hot0, hot1}},
			{Stage: "warm", Target: filepath.Join(tmp, "out-c"), Source: []string{warn0}},
		},
	}

	got := collectAllSrcGroupRoots(cfg, group)
	want := []string{
...
// (entry.Nodes drives root resolution via cfg.BackupDir).
...
func writeSidxAt(t *testing.T, segDir string, docs []*bluge.Document) {
	t.Helper()
	if err := os.MkdirAll(filepath.Join(segDir, directCopySidxDirName), storage.DirPerm); err != nil {
		t.Fatalf("mkdir sidx: %v", err)
	}
	w, err := bluge.OpenWriter(bluge.DefaultConfig(filepath.Join(segDir, directCopySidxDirName)))
	if err != nil {
...
func TestBuildGroupUnionSidx_DeduplicatesAcrossSegments(t *testing.T) {
	...
	segA := filepath.Join(srcGroupRoot, directCopySegPrefix+"20260101")
	segB := filepath.Join(srcGroupRoot, directCopySegPrefix+"20260102")
	segC := filepath.Join(srcGroupRoot, directCopySegPrefix+"20260103")
	...
	resultPath, err := buildGroupUnionSidx(context.Background(), []string{srcGroupRoot}, stagingPath)
	if err != nil {
...
func TestBuildGroupUnionSidx_MissingSrcRootIsNoop(t *testing.T) {
	stagingPath := filepath.Join(t.TempDir(), "union", "sidx")
	resultPath, err := buildGroupUnionSidx(context.Background(), []string{filepath.Join(t.TempDir(), "no-such-dir")}, stagingPath)
	if err != nil {
