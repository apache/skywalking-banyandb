...
func (s *standalone) FlagSet() *run.FlagSet {
	...
	flagS.IntVar(&s.option.mergePolicy.maxParts, "measure-max-merge-parts", s.option.mergePolicy.maxParts, "the maximum number of parts to merge at once")
	flagS.Float64Var(&s.option.mergePolicy.minMergeMultiplier, "measure-min-merge-multiplier", s.option.mergePolicy.minMergeMultiplier,
...
func (s *standalone) PreRun(ctx context.Context) error {
	...
	// queue's per-topic ResponseCodec dispatcher selects RawFrameCodec for
	// the vec raw columnar frame body — topic-AND-process-wire-mode
	// selection, G9f spec G9f.0.
	data.SetMeasureWireModeRaw(true)
	// Loud announcement so operators see the cluster wire mode at boot,
	// making the rollout state auditable from logs.
	s.l.Info().
