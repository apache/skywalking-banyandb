...
func (e *etcdSchemaRegistry) cleanupIndexRulesForDeletedTags(ctx context.Context, group, subjectName string, catalog commonv1.Catalog, deletedTags map[string]struct{}) error {
	...
	indexRules, err := e.ListIndexRule(ctx, ListOpt{Group: group})
	if err != nil {
		return err
	}
	rulesToDelete := make(map[string]struct{})
	for _, rule := range indexRules {
		for _, tagName := range rule.GetTags() {
			if _, deleted := deletedTags[tagName]; deleted {
				rulesToDelete[rule.GetMetadata().GetName()] = struct{}{}
				break
			}
		}
	}
	...
	bindings, err := e.ListIndexRuleBinding(ctx, ListOpt{Group: group})
	if err != nil {
		return err
	}
	for _, binding := range bindings {
		sub := binding.GetSubject()
		if sub.GetCatalog() != catalog || sub.GetName() != subjectName {
			continue
		}
		var remainingRules []string
		for _, ruleName := range binding.GetRules() {
			if _, shouldDelete := rulesToDelete[ruleName]; !shouldDelete {
				remainingRules = append(remainingRules, ruleName)
			}
		}
		if len(remainingRules) == 0 {
			if _, err := e.DeleteIndexRuleBinding(ctx, binding.GetMetadata()); err != nil {
				return err
			}
		} else {
			binding.Rules = remainingRules
			if err := e.UpdateIndexRuleBinding(ctx, binding); err != nil {
				return err
			}
		}
	}
