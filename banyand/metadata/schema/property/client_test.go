...
func createTestGroup(t *testing.T, reg *property.SchemaRegistry) {
	t.Helper()
	_, createGroupErr := reg.CreateGroup(context.Background(), testGroup())
...
func TestGroupCRUD(t *testing.T) {
	...
	g := testGroup()
	_, createGroupErr := reg.CreateGroup(ctx, g)
	require.NoError(t, createGroupErr)
	...
	_, updateGroupErr := reg.UpdateGroup(ctx, got)
	require.NoError(t, updateGroupErr)
	updated, getErr := reg.GetGroup(ctx, "test-group")
	require.NoError(t, getErr)
	assert.Equal(t, uint32(4), updated.GetResourceOpts().GetShardNum())
	deleted, _, deleteErr := reg.DeleteGroup(ctx, "test-group")
...
func TestStreamCRUD(t *testing.T) {
	...
	deleted, _, deleteErr := reg.DeleteStream(ctx, &commonv1.Metadata{Group: "test-group", Name: testStreamName})
	require.NoError(t, deleteErr)
	assert.True(t, deleted)
...
func TestGroupUpdateNoOpSkipsBroadcast(t *testing.T) {
	...
	_, updateNoOpErr := reg.UpdateGroup(ctx, got)
	require.NoError(t, updateNoOpErr)
	...
	time.Sleep(2 * time.Millisecond)
	_, updateRealErr := reg.UpdateGroup(ctx, afterNoOp)
	require.NoError(t, updateRealErr)
...
func TestTraceCRUD(t *testing.T) {
	...
	deleted, _, deleteErr := reg.DeleteTrace(ctx, &commonv1.Metadata{Group: "test-group", Name: "test-trace"})
	require.NoError(t, deleteErr)
	assert.True(t, deleted)
...
func TestIndexRuleCRUD(t *testing.T) {
	...
	_, createIRErr := reg.CreateIndexRule(ctx, ir)
	require.NoError(t, createIRErr)
	...
	_, updateIRErr := reg.UpdateIndexRule(ctx, got)
	require.NoError(t, updateIRErr)
	...
	deleted, _, deleteErr := reg.DeleteIndexRule(ctx, &commonv1.Metadata{Group: "test-group", Name: "test-index-rule"})
	require.NoError(t, deleteErr)
	assert.True(t, deleted)
...
func TestIndexRuleCRUD_AutoGenerateCRC32ID(t *testing.T) {
	...
	ir := testIndexRule("test-group")
	ir.Metadata.Id = 0
	_, createIRCRCErr := reg.CreateIndexRule(ctx, ir)
	require.NoError(t, createIRCRCErr)
	got, getErr := reg.GetIndexRule(ctx, &commonv1.Metadata{Group: "test-group", Name: "test-index-rule"})
	require.NoError(t, getErr)
...
func TestIndexRuleBindingCRUD(t *testing.T) {
	...
	_, createIRBErr := reg.CreateIndexRuleBinding(ctx, irb)
	require.NoError(t, createIRBErr)
	...
	_, updateIRBErr := reg.UpdateIndexRuleBinding(ctx, got)
	require.NoError(t, updateIRBErr)
	...
	deleted, _, deleteErr := reg.DeleteIndexRuleBinding(ctx, &commonv1.Metadata{Group: "test-group", Name: "test-index-rule-binding"})
	require.NoError(t, deleteErr)
	assert.True(t, deleted)
...
func TestTopNAggregationCRUD(t *testing.T) {
	...
	_, createTopNErr := reg.CreateTopNAggregation(ctx, topn)
	require.NoError(t, createTopNErr)
	...
	_, updateTopNErr := reg.UpdateTopNAggregation(ctx, got)
	require.NoError(t, updateTopNErr)
	...
	deleted, _, deleteErr := reg.DeleteTopNAggregation(ctx, &commonv1.Metadata{Group: "test-group", Name: "test-topn"})
	require.NoError(t, deleteErr)
	assert.True(t, deleted)
...
func TestPropertyCRUD(t *testing.T) {
	...
	reg := newTestRegistry(t, addr)
	ctx := context.Background()
	_, createGroupErr := reg.CreateGroup(ctx, testPropertyGroup("test-prop-group"))
	require.NoError(t, createGroupErr)
	...
	deleted, _, deleteErr := reg.DeleteProperty(ctx, &commonv1.Metadata{Group: "test-prop-group", Name: "test-property"})
	require.NoError(t, deleteErr)
	assert.True(t, deleted)
...
func TestMultiNodeBroadcastWrite(t *testing.T) {
	...
	_, _, deleteErr := reg.DeleteStream(ctx, &commonv1.Metadata{Group: "test-group", Name: testStreamName})
	require.NoError(t, deleteErr)
	_, getErr := reg.GetStream(ctx, &commonv1.Metadata{Group: "test-group", Name: testStreamName})
	require.Error(t, getErr)
}
...
func TestOnAddOrUpdate_IgnoresNonMetaRoleNode(t *testing.T) {
	...
	_, createGroupErr0 := reg.CreateGroup(ctx, testGroup())
	require.NoError(t, createGroupErr0)
	got, getErr := reg.GetGroup(ctx, "test-group")
	require.NoError(t, getErr)
	assert.Equal(t, "test-group", got.GetMetadata().GetName())
}
...
func TestOnAddOrUpdate_RepairOldDataToNewNode(t *testing.T) {
	...
	reg.RegisterHandler("test-handler", schema.KindGroup|schema.KindStream, handler)
	ctx := context.Background()
	_, createGroupErr1 := reg.CreateGroup(ctx, testGroup())
	require.NoError(t, createGroupErr1)
	s := testStream()
	_, createErr := reg.CreateStream(ctx, s)
	require.NoError(t, createErr)
...
func TestOnDelete_RemoveNode(t *testing.T) {
	...
	t.Cleanup(func() { _ = reg.Close() })
	ctx := context.Background()
	_, createGroupErr2 := reg.CreateGroup(ctx, testGroup())
	require.NoError(t, createGroupErr2)
	s := testStream()
	_, createErr := reg.CreateStream(ctx, s)
	require.NoError(t, createErr)
...
func TestTLS_GroupCRUD(t *testing.T) {
	...
	ctx := context.Background()
	g := testGroup()
	_, createGroupTLSErr := reg.CreateGroup(ctx, g)
	require.NoError(t, createGroupTLSErr)
	got, getErr := reg.GetGroup(ctx, "test-group")
	require.NoError(t, getErr)
	assert.Equal(t, "test-group", got.GetMetadata().GetName())
	groups, listErr := reg.ListGroup(ctx)
	...
	assert.Len(t, groups, 1)
	got.ResourceOpts.ShardNum = 4
	_, updateGroupTLSErr := reg.UpdateGroup(ctx, got)
	require.NoError(t, updateGroupTLSErr)
	updated, getErr := reg.GetGroup(ctx, "test-group")
	require.NoError(t, getErr)
	assert.Equal(t, uint32(4), updated.GetResourceOpts().GetShardNum())
	deleted, _, deleteGroupTLSErr := reg.DeleteGroup(ctx, "test-group")
	require.NoError(t, deleteGroupTLSErr)
	assert.True(t, deleted)
	_, getErr = reg.GetGroup(ctx, "test-group")
	require.Error(t, getErr)
	groups, listErr = reg.ListGroup(ctx)
