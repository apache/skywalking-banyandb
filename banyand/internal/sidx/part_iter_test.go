...
func TestPartIterVerification(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Step 4: Initialize partIter with clean blockMetadataArray
...
func TestPartIterEdgeCases(t *testing.T) {
	...
	t.Run("no matching key range", func(t *testing.T) {
		...
		bma.reset()
		pi.init(bma, part, []common.SeriesID{1}, 200, 300) // No overlap with key 100
	...
	t.Run("no matching series ID", func(t *testing.T) {
		...
		bma.reset()
		pi.init(bma, part, []common.SeriesID{2}, 0, 200) // Different series ID
