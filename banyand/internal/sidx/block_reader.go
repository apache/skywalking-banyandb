...
var seqReaderPool = pool.Register[*seqReader]("sidx-seqReader")

type seqReaders struct {
	tagMetadata map[string]*seqReader
	tagData     map[string]*seqReader
	tagFilters  map[string]*seqReader
	primary     seqReader
	keys        seqReader
	data        seqReader
}
...
func (sr *seqReaders) init(p *part) {
	...
	sr.data.init(p.data)
	if sr.tagData == nil {
		sr.tagData = make(map[string]*seqReader)
		sr.tagMetadata = make(map[string]*seqReader)
		sr.tagFilters = make(map[string]*seqReader)
	}
