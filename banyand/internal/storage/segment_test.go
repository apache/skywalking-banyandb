...
// newAlignmentTestController spins up a real segmentController backed by a
// temp dir with the given SegmentInterval, and returns the controller plus
// a cleanup function. Used by regression tests for segment.create.
...
// TestCreateSegment_OutOfOrderArrival_NoTruncation pins the cold-tier
// regression: when a later-timestamp part is processed first and a part
// with an earlier timestamp arrives afterwards, segment.create must produce
// a new segment with the full configured SegmentInterval span instead of
// truncating it against the existing later neighbor.
func TestCreateSegment_OutOfOrderArrival_NoTruncation(t *testing.T) {
	...
	assert.Equal(t, fifteenDays, seg1.End.Sub(seg1.Start),
		"first segment must span the full configured SegmentInterval")
	assert.Equal(t, fifteenDays, seg2.End.Sub(seg2.Start),
		"out-of-order second segment must also span the full SegmentInterval (no truncation)")
}
