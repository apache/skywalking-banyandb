...
- Take the native log group's shard count from storage rather than from this node's flag: a create is broadcast to every schema server and reports `AlreadyExists` only when all of them reject it, so a node could route events to shards the group does not have, where they were written, acknowledged, counted and then invisible after the next open. The startup and retry passes now read the group and stream before creating them, which also stops a settled cluster from re-broadcasting creates every 10s.
- Charge a native log event the memory its buffered write request holds, about 1.8KiB, instead of the length of its line, and report a budget refusal as `memory_pressure` rather than `buffer_full`. The buffer could hold ten times the configured budget, and `--logging-native-max-bytes` could not bind at all.
- Keep `_monitoring_log`, the log group's own storage, out of native collection whatever `--logging-native-exclude-modules` says, and ignore a blank entry in that list instead of treating it as a prefix that excludes every module.
- Store an event logged without a level as `none` and admit it as an informational event: the level is half the series key, so an empty value made such a row unfindable, and `NoLevel` outranks every real level, so these events ignored `--logging-native-level` entirely. A library writing through a standard `log.Logger`, as the index library does, produces them.
- Leave every logger's threshold alone on a role that cannot store its own logs. `--logging-native-enabled` on a liaison made the process encode lines that both the console gate and the empty sink discarded.
