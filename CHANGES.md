...
- Vectorized **stream** and **trace** query paths are now enabled by default, joining measure. `--stream-vectorized-enabled` and `--trace-vectorized-enabled` now default to `true`. Validated by a 48h containerized soak (parity pass, 84.6M spans, zero memory alerts) plus the standalone and multi-node distributed parity suites. Rollback: pass `--trace-vectorized-enabled=false` on the standalone or data-node command line and restart; the row path resumes immediately.
