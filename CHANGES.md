...
- Vectorized **stream** and **trace** query paths are now enabled by default, joining measure. `--stream-vectorized-enabled` and `--trace-vectorized-enabled` now default to `true`. Validated on two axes. A 48h single-node containerized soak per engine covers the compute path (stream 575/575 parity cycles and 84.75M spans; trace parity pass and 84.6M spans; zero memory alerts, no restarts, and TTL reclamation observed releasing memory proportionally to the data it cached). A 6h soak per engine on a 1 liaison + 2 data node cluster covers the wire path, which the single-node runs cannot reach at all because a data node only emits the columnar frame when it is distributed: stream 72/72 parity cycles with 600 frames encoded across the data nodes and 600 decoded on the liaison, trace 72/72 with 720/720, both with 10.5M spans written and no restarts. The standalone and multi-node distributed parity suites gate the same paths in CI. Rollback: pass `--stream-vectorized-enabled=false` / `--trace-vectorized-enabled=false` on the standalone or data-node command line and restart; the row path resumes immediately.
- Add the vectorized **stream** query path. Timestamp-ordered (asc/desc), index-ordered, criteria-filtered, and multi-group queries dispatch through the columnar pipeline (scan -> batch -> merge -> distinct -> limit) with a native columnar liaison<->data wire frame replacing per-element protobuf. Shapes the vec dispatch does not recognize -- and index-ordered queries whose sort tag is absent from the projection, where the OrderKey column cannot be populated -- fall back to the row path, which remains correct. Traced queries (`trace=true`) always take the protobuf egress, since the frame carries no channel for `common.v1.Trace`. Validated by a 48h containerized soak (575/575 parity cycles, zero divergences, 84.75M spans) plus standalone and multi-node distributed parity suites. The query-layer dedup treats `element_id` as identifying an element globally and holds its seen-set for the whole query, so two rows carrying one `element_id` collapse regardless of timestamp or which parts they came from — a stronger guarantee than the row path's per-merge-round seen-set, which only collapses duplicates landing in the same round.
...
- [Breaking Change] Enabling a vectorized path by default changes the required rolling-upgrade order: **upgrade "liaison" nodes BEFORE "data" nodes**, the reverse of the general strategy. Each `--*-vectorized-enabled` flag selects the liaison<->data wire format as well as the query engine -- a flag-on distributed data node emits a native columnar frame. A 0.11 liaison decodes both frames and protobuf (dispatching per message on the frame magic byte), but an older liaison has no frame decoder and cannot deserialize the response, so upgrading data nodes first fails queries for the duration of the rollout. Standalone deployments are unaffected (the frame is only emitted on a distributed data node). See [Upgrade Procedure](https://github.com/apache/skywalking-banyandb/tree/master/docs/operation/upgrade.md) for the ordering rule and the disable-then-enable alternative. This applies equally to the measure vectorized path, which has been default-on since 0.10.
...
- Fix the flaky stream snapshot test, which gated on the part directory appearing in `tab/` as the flush-completion signal. That directory is created by the first line of `memPart.mustFlush`, before the mem->file introduction reaches the snapshot and before the `.snp` manifest is persisted, so `TakeFileSnapshot` could run mid-flush and copy no part at all (and read a half-written manifest). It now waits for the persisted manifest, the same fix already applied to the measure snapshot tests in #1152; deleting the accompanying sleep reproduces the old failure every run.
...
- Reject non-matching tag entries in the sampler's rule matcher before the string comparison. Most entries belong to some other tag, so the common outcome is rejection; a length and first-byte check now precedes `strings.HasPrefix`, and rules naming the array column itself take a branch with no prefix handling at all. Tag-rule decisions drop a further 18% and full-scenario decisions 12-27%.
- Read the trace sampler's scalar tag columns without materializing the SDK's decoded value. `sdk.Value` is 112 bytes and `TagColumn.At` returns it by value, so reading the duration, start-time and error columns per row cost more in `runtime.duffcopy` than in decoding. The sampler now reads those 8-byte cells directly, gated narrowly on the column's exact type so a malformed cell in a wrong-typed column still fails open with the same error. Duration-envelope decisions drop 80% (`durationOnly` 15.2 -> 2.9 us), dedicated-error-column decisions 79% (`errorsOnly` 8.0 -> 1.7 us), and full-scenario decisions 17-34%.
- Decode escape-free trace tag rows without copying them. The sw/zipkin trace sampler copied every tag row before decoding, because the var-array decode rewrites its input in place and the engine hands the same batch to every link of a sampler chain. That copy is only needed for a row that actually contains an escape, so rows without one now decode straight from the block with the entries aliasing it — safe because `assembleTraceBlock` already deep-copies each value into block-owned storage that outlives an abandoned `Decide`. Such a row also skips the codec's redundant per-entry escape scan, and the per-row copy buffer, the stable buffer and their pool traffic are now acquired only when a row really is escaped. Tag-rule decisions drop 64% (`tagRulesOnly` 68.4 → 24.4 µs) and full-scenario decisions 35-45%.
- Precompute each tag rule's `"tagKey="` entry prefix at admission instead of rebuilding it on every `matchEntries` call (once per rule per trace).
- Add an escape-free fast path to `vararray.UnmarshalVarArray`, the var-array decoder shared by sidx, trace query, index filtering, the dump tool and the sampler SDK. An entry containing no escape byte is located with two `bytes.IndexByte` scans instead of a per-byte loop, dropping that case from O(len) to O(1) (-91% at a 256-byte payload) and leaving the buffer untouched. An entry that does carry an escape resumes the in-place loop at the first escape rather than restarting, since the bytes before it never need shifting — so a value with a single escape also improves (-89% at a 256-byte payload); only escape-dense values pay a net cost. Sampler tag-rule decisions drop 36% (`tagRulesOnly` 68.1 → 43.3 µs) and full-scenario decisions 19-25%. The decoder's documented mutation contract is now conditional: a caller may skip its defensive copy only behind its own escape check, never unconditionally.
