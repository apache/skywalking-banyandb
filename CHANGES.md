...
- Add reusable BydbQL binding: `Prepare` a query once, then `Bind` it many times without re-parsing or mutating the template. The liaison caches prepared statements on the gRPC query path (LRU, bounded by entry count and total bytes, enabled by default) so repeated query templates skip parsing; tune with `--bydbql-prepared-cache-size` / `--bydbql-prepared-cache-max-bytes` and observe via the `bydbql_prepared_cache_*` metrics (a hit/miss counter plus hit-ratio, count, and bytes gauges).
