...
- Refactor the lifecycle cycle-level metrics (`banyandb_lifecycle_cycles_total`, `banyandb_lifecycle_last_run_timestamp_seconds`, `banyandb_lifecycle_last_run_success`) to carry labels `remote_node`, `remote_role`, `remote_tier`, `group` — matching the per-message `banyandb_lifecycle_migration_*` family emitted by the queue/pub lifecycle publisher (file-sync path: `banyand/queue/pub/chunked_sync.go:67-82`; batch-write path: `banyand/queue/pub/batch.go:215, 271, 291-292, 421, 471-472, 488, 511, 520, 532`). The labels are sourced from the (senderNode, "lifecycle", senderTier) tuple that `parseGroup` already computes via `resolveSelfIdentity` and stamps onto the wire via `SetSelfNode`; the per-group `recordCycleGroup` helper is called from each `process*Group` site to Inc `cycles_total` and Set `last_run_timestamp_seconds` with the (group, remote_*) tuple, and the deferred `recordLastRun` continues to Set `last_run_success` and `last_run_timestamp_seconds` at cycle end with the cycle's last-seen tuple (empty labels on the empty-cycle path). Dashboards can now cross-join the cycle-level series with the per-message pub series on the same (group, node, role, tier) keys. [Breaking Change] Update any alert/panel that pinned the unlabeld form (e.g. `banyandb_lifecycle_last_run_success == 1`) to use the new label-superset form (`banyandb_lifecycle_last_run_success{group!=""}`).
